...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Tipos de Produto</h1>
                <p class="text-gray-600 mt-1">Gerencie as categorias dos produtos</p>
            </div>
            <a href="{{ url('types/new') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors">
                        ...
                        @foreach($types as $type)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $type->products->count() }} produto{{ $type->products->count() !== 1 ? 's' : '' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <a href="{{ url('/types/update', ['id' => $type->id]) }}" class="text-blue-600 hover:text-blue-900 transition-colors">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                        </a>
                                        <button onclick="confirmDelete('{{ url('/types/delete', ['id' => $type->id]) }}')" class="text-red-600 hover:text-red-900 transition-colors">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="mt-2 text-sm text-gray-500">Tem certeza de que deseja excluir este tipo? Esta ação não pode ser desfeita.</p>
                    </div>
                </div>
            </div>
